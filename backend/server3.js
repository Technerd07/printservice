...
// ==========================================
// Enhanced Master Key Management
// ==========================================

// Secure key derivation configuration
const KDF_ITERATIONS = 100000; // Number of iterations for key derivation
const KDF_KEY_LENGTH = 32; // Desired key length in bytes
const KDF_SALT_LENGTH = 32; // Salt length in bytes
const KDF_ALGORITHM = 'sha256'; // Hash algorithm for key derivation

// Your powerful constant phrase (change this to your own secure phrase)
// This should be a long, complex phrase that only you know
const MASTER_PHRASE = process.env.MASTER_PHRASE || 'PrintEase2023!SecureEncryptionKeyPhrase#Complexity@Maximum';

// Salt file path for key derivation
const SALT_FILE_PATH = path.join(LOG_DIR, '.key_salt');

let MASTER_KEY;

/**
 * Derive a secure key from a phrase using PBKDF2
 * @param {string} phrase - The master phrase
 * @param {Buffer} salt - The salt to use
 * @returns {Buffer} - The derived key
 */
function deriveKeyFromPhrase(phrase, salt) {
  return crypto.pbkdf2Sync(phrase, salt, KDF_ITERATIONS, KDF_KEY_LENGTH, KDF_ALGORITHM);
}

/**
 * Generate or load a salt for key derivation
 * @returns {Promise<Buffer>} - The salt
 */
async function getOrCreateSalt() {
  try {
    // Try to read existing salt
    const saltData = await fs.readFile(SALT_FILE_PATH);
    logger.audit('SALT_LOADED', { path: SALT_FILE_PATH });
    return saltData;
  } catch (err) {
    // Generate a new salt if one doesn't exist
    const salt = crypto.randomBytes(KDF_SALT_LENGTH);
    await fs.writeFile(SALT_FILE_PATH, salt, { mode: 0o600 });
    logger.audit('SALT_CREATED', { path: SALT_FILE_PATH });
    return salt;
  }
}

/**
 * Initialize the master key from a phrase
 */
async function initializeMasterKey() {
  try {
    ...
    // Derive the master key from phrase and salt
    MASTER_KEY = deriveKeyFromPhrase(MASTER_PHRASE, salt);
    
    logger.audit('MASTER_KEY_DERIVED', { 
      method: 'PBKDF2',
      iterations: KDF_ITERATIONS,
      keyLength: KDF_KEY_LENGTH * 8,
      algorithm: KDF_ALGORITHM
    });
    
    console.log('✓ MASTER_KEY derived from phrase using PBKDF2');
    console.log(`  🔐 Key Derivation: ${KDF_ITERATIONS} iterations of ${KDF_ALGORITHM}`);
    console.log(`  🧂 Salt: ${salt.toString('hex').slice(0, 16)}...`);
    
    return true;
  } catch (err) {
    logger.error('MASTER_KEY_DERIVATION_FAILED', {
      error: err.message,
      stack: err.stack
    });
    console.error('❌ Failed to derive master key. Exiting.');
    process.exit(1);
  }
}
...
const server = app.listen(APP_PORT, HOST, () => {
  ...
  console.log(`  🔐 Key Derivation: PBKDF2 with ${KDF_ITERATIONS} iterations`);
